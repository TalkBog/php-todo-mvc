  ...
  <form method="post" class="flex-1 flex flex-col space-y-4">
    ...
    <label class="select-none">
      ...
      <input
        ...
        value="<?php echo $editing? $task->getTitle(): null?>"
      />
    </label>
    ...
    <label class="select-none">
      ...
      <textarea name="content" placeholder="Contenu..." class="appearance-none block w-full bg-slate-100 text-slate-700 border border-slate-100 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-slate-400 min-h-[200px]"><?php echo $editing? $task->getDescription(): ""?></textarea>
    </label>
    
    <!-- completed -->
    <div class="pt-2">
      <label class="inline-flex relative items-center cursor-pointer">
        <input type="checkbox" name="completed" class="sr-only peer" <?php if($editing && $task->isCompleted()){echo "checked";}?>>
